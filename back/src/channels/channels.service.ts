...
export class ChannelsService {
  ...
  async allChannelList() {
    ...
    const ret = await this.chatchannelRepository.find({
      select: ["id", "name", "type", "authId", "createdAt", "updatedAt", "deleteAt"], order: {
        createdAt: "DESC",
      }
    });
    return ret;
  ...
  async getAllMessage(channelId: number, userId: string) {
    ...
    const chatContent = await this.chatcontentRepository.find({
      where: { channelId }, select: ["userId", "message", "updatedAt"], order: {
        createdAt: "DESC",
      }
    })
    return chatContent;
    ...
    setTimeout(() => {
      (async () => {
        await this.chatmemberRepository.update({
          channelId, userId: muteId
        }, { mute: false }).then(() => { this.eventsGateway.server.to(`channel-${channelId}`).emit('mute', null); })
      })();
    }, time * 1000);
