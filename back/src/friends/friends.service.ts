...
export class FriendsService {
  ...
  async addFriend(userId1: string, userId2: string) {
    if (this.checkFriend(userId1, userId2))
      throw new ForbiddenException('이미 친구 관계');
    if (this.checkBlock(userId1, userId2) || this.checkBlock(userId2, userId1))
      throw new ForbiddenException('Block 상태');    
    const newFriend1 = new Friend();
    newFriend1.userId2 = userId1;
  ...
  async removeFriend(userId1: string, userId2: string) {
    if (this.checkFriend(userId1, userId2) && this.checkFriend(userId2, userId1)) {
